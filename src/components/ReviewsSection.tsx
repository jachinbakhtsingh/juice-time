      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-12 sm:mb-16 gap-6 sm:gap-8">
          ...
          <div className="flex items-center gap-5">
            <div className="flex flex-col">
              ...
              <span className="text-xs sm:text-sm font-bold">4.9 / 5.0 Average Rating</span>
              <span className="text-[11px] sm:text-xs text-neutral-400">Based on 42,800+ verified customer reviews</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-12 sm:mb-16">
          {PRESS_LOGOS.map((press) => (
            <div
              key={press.name}
              className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border text-center flex flex-col justify-center items-center ${
                isDark ? 'bg-neutral-950/70 border-neutral-800' : 'bg-neutral-50 border-neutral-200'
              }`}
            >
              <span className="font-['Syne'] font-black tracking-widest text-sm sm:text-base lg:text-lg mb-1.5 sm:mb-2 text-white">
                {press.name}
              </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {REVIEWS_DATA.map((rev) => (
            <div
              key={rev.id}
              className={`p-5 sm:p-8 rounded-xl sm:rounded-2xl border transition-all duration-300 relative ${
                isDark
            ...
            >
              ...
              <h4 className="text-base sm:text-lg font-bold mb-2 text-white">"{rev.title}"</h4>
              <p className="text-xs sm:text-sm text-neutral-400 leading-relaxed mb-4 sm:mb-6">
                {rev.content}
              </p>

              <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-neutral-800/60 text-xs">
                ...
                <span
                  className="px-2 sm:px-2.5 py-0.5 rounded-full text-[9px] sm:text-[10px] font-mono uppercase tracking-wider font-semibold"
                  style={{
